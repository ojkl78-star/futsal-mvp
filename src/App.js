...
import { Plus, Trash2, Users, ChevronDown, X, Save, BarChart2, Trophy } from "lucide-react";
...
const RESULT_MAP = {
  승: 1, 승리: 1, win: 1, WIN: 1,
  무: 0, 무승부: 0, draw: 0, DRAW: 0,
  패: -1, 패배: -1, lose: -1, LOSE: -1,
};
...
const parseResult = (v) => {
  ...
  const n = Number(v);
  if (!isNaN(n)) return n;
  return RESULT_MAP[String(v).trim()] ?? null;
};
...
const EMPTY_GAME = () => ({
  ...
  type: "매치",
  player: "",
  goal: 0, assist: 0, save: 0, matchMvp: 0,
  result: "",
  note: "",
});
...
function calcStats(games, playerName) {
  ...
  let goal = 0, assist = 0, save = 0, matchMvp = 0, matchTotal = 0;
  let selfResult = 0, matchCount = 0, selfCount = 0;

  rows.forEach(g => {
    if (g.type === "매치") {
      goal += Number(g.goal) || 0;
      assist += Number(g.assist) || 0;
      save += Number(g.save) || 0;
      matchMvp += Number(g.matchMvp) || 0;
      matchTotal += (Number(g.goal)||0)+(Number(g.assist)||0)+(Number(g.save)||0)+(Number(g.matchMvp)||0);
      matchCount++;
    } else {
      const r = parseResult(g.result);
      if (r !== null) { selfResult += r; selfCount++; }
    }
  });

  return { goal, assist, save, matchMvp, matchTotal, selfResult, total: matchTotal + selfResult, matchCount, selfCount, gameCount: rows.length };
}

const inp = {
  background: "#111827", border: "1px solid #1e2d45", borderRadius: 8,
  color: "#e8f0fe", padding: "8px 12px", fontSize: 13, outline: "none",
...
export default function App() {
  ...
  // Firebase 실시간 연결
  useEffect(() => {
    const unsubP = onValue(ref(db, "players"), snap => {
      setPlayers(snap.val() || {});
      setLoading(false);
    });
    const unsubG = onValue(ref(db, "games"), snap => {
      setGames(snap.val() || {});
    });
  ...
  const addPlayer = async () => {
    ...
    if (!name) return;
    if (Object.values(players).find(p => p.name === name)) return;
    setSyncing(true);
  ...
  const removePlayer = async (id) => {
    ...
    // 해당 선수 경기 기록도 삭제
    const toDelete = Object.entries(games).filter(([, g]) => g.player === name);
    for (const [gid] of toDelete) await remove(ref(db, `games/${gid}`));
  };
  ...
  const ranking = Object.entries(players)
    .map(([id, p]) => ({ id, ...p, ...calcStats(games, p.name) }))
    .sort((a, b) => b.total - a.total);

  const filteredGames = Object.entries(games)
    .filter(([, g]) => filterType === "전체" || g.type === filterType)
  ...
  if (loading) return (
    <div style={{ background: C.bg, minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:12 }}>
      ...
      <div style={{ color:C.accent, fontSize:14, fontFamily:"monospace", letterSpacing:2 }}>LOADING...</div>
    </div>
  );
    ...
    <div style={{ background:C.bg, minHeight:"100vh", fontFamily:"'Noto Sans KR', sans-serif", color:C.text }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#0d1b2e,#0a0e1a)", borderBottom:`1px solid ${C.border}`, padding:"0 20px", position:"sticky", top:0, zIndex:100 }}>
        <div style={{ maxWidth:900, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", height:58 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div>
              <div style={{ fontWeight:800, fontSize:16, letterSpacing:-0.5 }}>풋살 동호회</div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            <span style={{ fontSize:11, color:C.muted }}>{playerList.length}명 · {gameCount}경기</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ background:C.surface, borderBottom:`1px solid ${C.border}` }}>
        <div style={{ maxWidth:900, margin:"0 auto", display:"flex", padding:"0 20px" }}>
          {[
            { id:"ranking", label:"🏆 순위" },
            { id:"games",   label:"📋 경기 기록" },
            { id:"players", label:"👥 선수 관리" },
          ].map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              background:"none", border:"none", cursor:"pointer",
              padding:"14px 16px", fontSize:13, fontWeight: tab===t.id ? 700 : 400,
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {ranking.length === 0 ? (
              <div style={{ textAlign:"center", color:C.muted, padding:60 }}>
                ...
                <div>선수를 먼저 등록해주세요</div>
              </div>
            ) : ranking.map((p, i) => {
              const medal = i===0?"🥇":i===1?"🥈":i===2?"🥉":`${i+1}`;
              const highlight = i===0?C.gold:i===1?"#c0c0c0":i===2?"#cd7f32":C.accent;
              const isExp = expandedPlayer === p.id;
              return (
                <div key={p.id} onClick={() => setExpandedPlayer(isExp ? null : p.id)} style={{
                  background:C.card, border:`1px solid ${i<3?highlight+"44":C.border}`,
                  borderRadius:14, padding:"16px 20px", cursor:"pointer",
                  boxShadow: i===0?`0 0 20px rgba(255,215,0,0.15)`:"none",
                }}>
                  <div style={{ display:"flex", alignItems:"center", gap:14 }}>
                    <span style={{ fontSize:i<3?26:14, minWidth:30, textAlign:"center", color:C.muted, fontWeight:700 }}>{medal}</span>
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:700, fontSize:16 }}>{p.name}</div>
                      <div style={{ color:C.muted, fontSize:12, marginTop:2 }}>매치 {p.matchCount}경기 · 자체전 {p.selfCount}경기</div>
                    </div>
                    <div style={{ textAlign:"right" }}>
                      <div style={{ fontSize:28, fontWeight:900, color:highlight, lineHeight:1 }}>{p.total}</div>
                  ...
                  {isExp && (
                    <div style={{ marginTop:16, paddingTop:16, borderTop:`1px solid ${C.border}` }}>
                      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                        ...
                        <StatPill label="📋 매치합" value={p.matchTotal} />
        ...
        {tab === "games" && (
          <div>
              ...
              <div style={{ display:"flex", gap:6 }}>
                {["전체","매치","자체전"].map(f => (
                  <button key={f} onClick={() => setFilterType(f)} style={{
                    background: filterType===f ? C.accent : C.card,
                    color: filterType===f ? C.bg : C.muted,
                    border:`1px solid ${filterType===f ? C.accent : C.border}`,
                    borderRadius:20, padding:"5px 14px", fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit",
                  }}>{f}</button>
                ))}
              </div>
              <button onClick={() => { setAddingGame(true); setNewGame(EMPTY_GAME()); }} style={{
                background:C.accent, color:C.bg, border:"none", borderRadius:8,
                padding:"8px 16px", fontSize:13, fontWeight:700, cursor:"pointer",
                display:"flex", alignItems:"center", gap:6, fontFamily:"inherit",
              }}><Plus size={14} /> 경기 추가</button>
            </div>
              ...
              <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:20, marginBottom:16 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom:12 }}>
                  ...
                  <div>
                    ...
                    <input type="date" value={newGame.date} onChange={e => setNewGame(g=>({...g,date:e.target.value}))} style={inp} />
                  </div>
                  <div>
                    <label style={lbl}>경기 유형</label>
                ...
                {newGame.type === "매치" ? (
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:12, marginBottom:12 }}>
                    {[["goal","⚽ 골득점"],["assist","🎯 골어시"],["save","🧤 선방"],["matchMvp","⭐ MVP"]].map(([k,l]) => (
                      <div key={k}>
                        ...
                        <input type="number" min="0" value={newGame[k]} onChange={e=>setNewGame(g=>({...g,[k]:e.target.value}))} style={inp} placeholder="0" />
                      </div>
                    ))}
                  </div>
                ) : (
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:12 }}>
                    <div>
                      <label style={lbl}>🏟️ 결과</label>
                      <select value={newGame.result} onChange={e=>setNewGame(g=>({...g,result:e.target.value}))} style={inp}>
                        <option value="">선택</option>
                        <option value="승">승 (+1점)</option>
                        <option value="무">무 (0점)</option>
                        <option value="패">패 (-1점)</option>
                      </select>
                    </div>
                    <div style={{ display:"flex", alignItems:"flex-end" }}>
                      <div style={{
                        background:C.surface, border:`1px solid ${C.border}`, borderRadius:8,
                        padding:"8px 16px", fontSize:20, fontWeight:800, width:"100%", textAlign:"center",
                        color: newGame.result==="승"?C.green:newGame.result==="패"?C.red:C.muted,
                      }}>
                ...
                <div style={{ marginBottom:12 }}>
                  ...
                  <input value={newGame.note} onChange={e=>setNewGame(g=>({...g,note:e.target.value}))} style={inp} placeholder="메모 (선택)" />
                </div>
            ...
            {filteredGames.length === 0 ? (
              ...
              const r = parseResult(g.result);
              const score = g.type==="매치"
                ? (Number(g.goal)||0)+(Number(g.assist)||0)+(Number(g.save)||0)+(Number(g.matchMvp)||0)
                : (r !== null ? r : 0);
              const scoreColor = g.type==="매치" ? C.accent : score>0?C.green:score<0?C.red:C.muted;
              return (
                <div key={id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"12px 16px", display:"flex", alignItems:"center", gap:12, marginBottom:8 }}>
                  <div style={{
                    background: g.type==="매치"?"rgba(0,212,255,0.1)":"rgba(0,230,118,0.1)",
                    border:`1px solid ${g.type==="매치"?C.accent+"44":C.green+"44"}`,
                    borderRadius:6, padding:"3px 8px", fontSize:11, fontWeight:700,
                    color: g.type==="매치"?C.accent:C.green, whiteSpace:"nowrap",
                  }}>{g.type}</div>
                  <div style={{ flex:1 }}>
                    <div style={{ fontWeight:600, fontSize:14 }}>{g.player}</div>
                    <div style={{ color:C.muted, fontSize:11, marginTop:2 }}>
                      {g.date}
                      {g.type==="매치" && ` · 골${g.goal||0} 어시${g.assist||0} 선방${g.save||0} MVP${g.matchMvp||0}`}
                      {g.type==="자체전" && ` · ${g.result||"–"}`}
                      {g.note && ` · ${g.note}`}
                    </div>
                  </div>
                  <div style={{ fontSize:20, fontWeight:800, color:scoreColor, minWidth:36, textAlign:"right" }}>
                    {g.type==="자체전" && score>=0 ? `+${score}` : score}
                  </div>
                  <button onClick={() => removeGame(id)} style={{ background:"none", border:"none", cursor:"pointer", color:C.muted, padding:4 }}>
                    <Trash2 size={14} />
                  </button>
                </div>
              );
            ...
            {playerList.length === 0 ? (
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                {Object.entries(players).map(([id, p]) => {
                  const s = calcStats(games, p.name);
                  return (
                    <div key={id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 16px", display:"flex", alignItems:"center", gap:12 }}>
                      <div style={{
                        ...
                        background:`linear-gradient(135deg,${C.accent}33,rgba(0,212,255,0.05))`,
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ color:C.muted, fontSize:11 }}>{s.gameCount}경기 · {s.total}점</div>
                      </div>
                      <button onClick={() => removePlayer(id)} style={{ background:"none", border:"none", cursor:"pointer", color:C.muted }}>
                        <Trash2 size={14} />
                      </button>
                    </div>
                  );
