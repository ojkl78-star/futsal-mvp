...
import { Plus, Trash2, ChevronDown, X, Save, Calendar } from "lucide-react";
...
export default function App() {
  ...
  const [attendChecked, setAttendChecked] = useState({});
  ...
  const saveGame = async () => {
    ...
    setSyncing(true); await push(ref(db,"games"),{...newGame,createdAt:Date.now()}); setAddingGame(false); setNewGame(EMPTY_GAME()); setSyncing(false);
  };

  const saveAttendance = async () => {
    setSyncing(true);
    for(const [name,checked] of Object.entries(attendChecked)) {
      if(!checked) continue;
      const already=Object.values(attendances).find(a=>a.playerName===name&&a.date===attendDate);
      if(!already) await push(ref(db,"attendances"),{playerName:name,date:attendDate,createdAt:Date.now()});
    }
    setAddingAttendance(false); setAttendChecked({}); setSyncing(false);
  };
  ...
  const attendByDate = Object.entries(attendances).reduce((acc,[id,a])=>{if(!acc[a.date])acc[a.date]=[];acc[a.date].push({id,...a});return acc;},{});
    ...
    <div style={{ background:C.bg, minHeight:"100vh", fontFamily:"'Noto Sans KR',sans-serif", color:C.text }}>
      ...
      <div style={{ background:C.surface, borderBottom:`1px solid ${C.border}` }}>
        <div style={{ maxWidth:900, margin:"0 auto", display:"flex", padding:"0 20px" }}>
          {[{id:"ranking",label:"🏆 순위"},{id:"games",label:"📋 경기 기록"},{id:"attendance",label:"📅 출석 관리"},{id:"players",label:"👥 선수 관리"}].map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)} style={{ background:"none",border:"none",cursor:"pointer",padding:"14px 12px",fontSize:12,fontWeight:tab===t.id?700:400,color:tab===t.id?C.accent:C.muted,borderBottom:tab===t.id?`2px solid ${C.accent}`:"2px solid transparent",transition:"all 0.2s",fontFamily:"inherit",whiteSpace:"nowrap" }}>{t.label}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px" }}>
        ...
        {tab==="ranking" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"10px 16px", marginBottom:4 }}>
              <span style={{ fontSize:11, color:C.muted }}>📋 매치/자체전 MVP 점수  +  📅 출석 점수 합산 순위</span>
            </div>
            {ranking.length===0 ? <div style={{ textAlign:"center",color:C.muted,padding:60 }}><div style={{ fontSize:48,marginBottom:12 }}>🏆</div><div>선수를 먼저 등록해주세요</div></div>
            : ranking.map((p,i)=>{
        ...
        {tab==="games" && (
          <div>
            ...
            {addingGame && (
              <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:20,marginBottom:16 }}>
                ...
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12 }}>
                  <div><label style={lbl}>선수</label><select value={newGame.player} onChange={e=>setNewGame(g=>({...g,player:e.target.value}))} style={inp}><option value="">선택</option>{Object.values(players).map(p=><option key={p.name}>{p.name}</option>)}</select></div>
                  ...
                  <div><label style={lbl}>경기 유형</label><select value={newGame.type} onChange={e=>setNewGame(g=>({...g,type:e.target.value}))} style={inp}><option>매치</option><option>자체전</option></select></div>
                </div>
                {newGame.type==="매치" ? (
                  ...
                  <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12 }}>
                    <div><label style={lbl}>🏟️ 결과</label><select value={newGame.result} onChange={e=>setNewGame(g=>({...g,result:e.target.value}))} style={inp}><option value="">선택</option><option value="승">승 (+1점)</option><option value="무">무 (0점)</option><option value="패">패 (-1점)</option></select></div>
                    <div style={{ display:"flex",alignItems:"flex-end" }}><div style={{ background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 16px",fontSize:20,fontWeight:800,width:"100%",textAlign:"center",color:newGame.result==="승"?C.green:newGame.result==="패"?C.red:C.muted }}>{newGame.result==="승"?"+1":newGame.result==="패"?"-1":newGame.result==="무"?"0":"–"}</div></div>
                  </div>
                )}
            ...
            {filteredGames.length===0 ? <div style={{ textAlign:"center",color:C.muted,padding:60 }}>경기 기록이 없습니다</div>
            : filteredGames.map(([id,g])=>{
              ...
              return (
                <div key={id} style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 16px",display:"flex",alignItems:"center",gap:12,marginBottom:8 }}>
                  ...
                  <button onClick={()=>remove(ref(db,`games/${id}`))} style={{ background:"none",border:"none",cursor:"pointer",color:C.muted,padding:4 }}><Trash2 size={14}/></button>
                </div>
              );
            })}
          </div>
        ...
        {tab==="attendance" && (
          <div>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16 }}>
              <span style={{ color:C.muted,fontSize:13 }}>경기 참석 1회 = +1점, 자동으로 순위에 반영</span>
              <button onClick={()=>{setAddingAttendance(true);setAttendDate(new Date().toISOString().split("T")[0]);setAttendChecked({});}} style={{ background:C.orange,color:"#000",border:"none",borderRadius:8,padding:"8px 16px",fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:6,fontFamily:"inherit" }}><Calendar size={14}/> 출석 체크</button>
            </div>

            {addingAttendance && (
              <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:20,marginBottom:16 }}>
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16 }}>
                  <span style={{ fontWeight:700,fontSize:15 }}>출석 체크</span>
                  <button onClick={()=>setAddingAttendance(false)} style={{ background:"none",border:"none",cursor:"pointer",color:C.muted }}><X size={18}/></button>
                </div>
                <div style={{ marginBottom:16 }}>
                  <label style={lbl}>경기 날짜</label>
                  <input type="date" value={attendDate} onChange={e=>setAttendDate(e.target.value)} style={{ ...inp,width:"auto" }}/>
                </div>
                <label style={lbl}>참석 선수 선택</label>
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16 }}>
                  {Object.values(players).map(p=>{
                    const checked=!!attendChecked[p.name];
                    const done=Object.values(attendances).find(a=>a.playerName===p.name&&a.date===attendDate);
                    return (
                      <div key={p.name} onClick={()=>!done&&setAttendChecked(prev=>({...prev,[p.name]:!prev[p.name]}))} style={{ background:done?"rgba(0,230,118,0.08)":checked?"rgba(255,152,0,0.12)":C.surface,border:`1px solid ${done?C.green+"44":checked?C.orange:C.border}`,borderRadius:10,padding:"10px 14px",display:"flex",alignItems:"center",gap:10,cursor:done?"default":"pointer" }}>
                        <div style={{ width:20,height:20,borderRadius:4,border:`2px solid ${done?C.green:checked?C.orange:C.muted}`,background:(checked||done)?(done?C.green:C.orange):"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>
                          {(checked||done)&&<span style={{ fontSize:12,color:"#000",fontWeight:900 }}>✓</span>}
                        </div>
                        <span style={{ fontWeight:600,fontSize:13,color:done?C.green:C.text }}>{p.name}</span>
                        {done&&<span style={{ fontSize:10,color:C.green,marginLeft:"auto" }}>완료</span>}
                      </div>
                    );
                  })}
                </div>
                <button onClick={saveAttendance} style={{ background:C.orange,color:"#000",border:"none",borderRadius:8,padding:"10px 24px",fontSize:14,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:6,fontFamily:"inherit" }}><Save size={14}/> 출석 저장</button>
              </div>
            )}

            {Object.keys(attendByDate).length===0 ? (
              <div style={{ textAlign:"center",color:C.muted,padding:60 }}><div style={{ fontSize:40,marginBottom:12 }}>📅</div><div>출석 기록이 없습니다</div></div>
            ) : Object.entries(attendByDate).sort(([a],[b])=>b.localeCompare(a)).map(([date,list])=>(
              <div key={date} style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px 16px",marginBottom:10 }}>
                <div style={{ fontWeight:700,fontSize:14,marginBottom:10,color:C.orange }}>📅 {date} · {list.length}명 참석</div>
                <div style={{ display:"flex",flexWrap:"wrap",gap:8 }}>
                  {list.map(a=>(
                    <div key={a.id} style={{ background:"rgba(255,152,0,0.1)",border:`1px solid ${C.orange}44`,borderRadius:20,padding:"4px 12px",display:"flex",alignItems:"center",gap:8 }}>
                      <span style={{ fontSize:13,fontWeight:600 }}>{a.playerName}</span>
                      <span style={{ fontSize:11,color:C.orange }}>+1</span>
                      <button onClick={()=>remove(ref(db,`attendances/${a.id}`))} style={{ background:"none",border:"none",cursor:"pointer",color:C.muted,padding:0,display:"flex" }}><X size={12}/></button>
                    </div>
                  ))}
                </div>
              </div>
